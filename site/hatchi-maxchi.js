...
(function () {
  ...
  var COMMAND_ITEMS = [
    { group: "Workspaces", label: "Operations Dashboard",
      html: '<button type="button" class="command__item" role="option">{i:layout-dashboard}<span>Operations Dashboard</span></button>' },
    { group: "Workspaces", label: "Platform Admin",
      html: '<button type="button" class="command__item" role="option">{i:settings}<span>Platform Admin</span></button>' },
    { group: "Records", label: "Invoices",
      html: '<button type="button" class="command__item" role="option">{i:receipt}<span>Invoices</span></button>' },
    { group: "Records", label: "Customers",
      html: '<button type="button" class="command__item" role="option">{i:users}<span>Customers</span></button>' },
    { group: "Records", label: "Alerts",
      html: '<button type="button" class="command__item" role="option">{i:triangle-alert}<span>Alerts</span></button>' }
  ];
  function renderCommandResults(url) {
    ...
    if (!hits.length) {
      return '<div class="command__empty">No matching destinations.</div>';
    }
    var html = "", g = "";
    hits.forEach(function (e) {
      if (e.group !== g) {
        g = e.group;
        html += '<div class="command__group">' + g + "</div>";
      }
      html += e.html;
    });
    return html;
  ...
  var TYPEAHEAD_ITEMS = [
    { hay: "aurora energy ltd company no. 09182736 utilities",
      html: '<div class="search-result-row" role="option" tabindex="-1" data-result-id="co-aurora" '
    ...
    { hay: "aurora foods plc company no. 10448291 fmcg",
      html: '<div class="search-result-row" role="option" tabindex="-1" data-result-id="co-foods" '
    ...
    { hay: "jordan dias jordan@acme.example ops lead",
      html: '<div class="search-result-row" role="option" tabindex="-1" data-result-id="user-jd" '
    ...
    { hay: "aisha khan aisha@acme.example finance",
      html: '<div class="search-result-row" role="option" tabindex="-1" data-result-id="user-ak" '
    ...
    { hay: "sensor pack sp-42 sku in stock 14",
      html: '<div class="search-result-row" role="option" tabindex="-1" data-result-id="sku-42" '
  ...
  function renderTypeaheadResults(url) {
    var q = (parseQuery(url).q || "").trim().toLowerCase();
    if (!q) {
      return '<div class="search-select-prompt" role="option" aria-disabled="true">'
    ...
    if (!hits.length) {
      return '<div class="search-result-empty">No results found for "'
...
(function () {
  ...
  function capOf(stack) {
    var raw = stack.getAttribute("data-toast-cap");
  ...
  function scan(root) {
    ...
    scope.querySelectorAll("[data-remove-after]").forEach(function (el) {
      if (el.classList && el.classList.contains("toast")) {
        ...
        setTimeout(function () {
          el.remove();
        }, parseDelayMs(el.getAttribute("data-remove-after")));
      }
    });
    if (scope !== document) {
      document.querySelectorAll(".toast[data-remove-after]").forEach(scheduleOne);
    }
  }
    ...
    if (actions && actions.length) {
      ...
      for (var i = 0; i < actions.length; i++) {
        ...
        if (a.href) {
          ...
          node.type = "button";
          node.className = "toast__action";
          node.setAttribute("data-toast-dismiss", "");
          node.textContent = String(label);
        }
...
(function () {
  ...
  var ITEM_SEL = ".master-detail__item";

  function rootOf(el) {
    if (!el || !el.closest) return null;
    return (
      el.closest("[data-master-detail]") ||
      el.closest(".master-detail")
    );
  }
...
(function () {
  ...
  function parts(el) {
    if (!el || !el.closest) return null;
    var group =
      el.closest(".form-slider-group") || el.closest(".form-slider-group");
    if (
      !group &&
      el.matches &&
      el.matches('input[type="range"][data-slider]')
    ) {
      group = el.parentElement;
    }
    if (!group) return null;
    if (group.closest && group.closest("[data-widget]")) return null;
    var range = group.querySelector('input[type="range"][data-slider]');
    var out = group.querySelector("[data-range-value]");
...
/* ── controllers/carousel.js ── */
/* HYPERPART: carousel */
/*
 * carousel — stage navigation for a server-marked strip.
 *
 * Contract: contracts/carousel.py · Decision 0009 (stage, wrap, autoplay).
 *
 *   root:     [data-carousel]
 *             data-carousel-index
 *             data-carousel-wrap = none|loop  (default none = clamp)
 *             data-carousel-interval = ms     (absent = no autoplay)
 *   slides:   .carousel__slide  — visible when [data-active]
 *   prev:     [data-carousel-prev]
 *   next:     [data-carousel-next]
 *   dots:     .carousel__dot + aria-current
 *   status:   [data-carousel-status]  — "Slide N of M" (+ autoplay hint)
 *
...
(function () {
  ...
  function rootOf(el) {
    if (!el || !el.closest) return null;
    return el.closest("[data-carousel]") || el.closest(".carousel");
  }

  function slideList(root) {
    return root.querySelectorAll(".carousel__slide");
  }

  function dotList(root) {
    return root.querySelectorAll(".carousel__dot");
  }

  function isActive(slide) {
    return (
      slide.hasAttribute("data-active") || slide.hasAttribute("data-active")
    );
  }
  ...
  document.addEventListener("click", function (evt) {
    ...
    var prevBtn =
      t.closest("[data-carousel-prev]") || t.closest("[data-carousel-prev]");
    var nextBtn =
      t.closest("[data-carousel-next]") || t.closest("[data-carousel-next]");
    var dot = t.closest(".carousel__dot") || t.closest(".carousel__dot");
  ...
  function rearmAll() {
    var roots = document.querySelectorAll("[data-carousel], [data-carousel]");
...
(function () {
  ...
  function buildQuery(root) {
    ...
    var parsedPage = parseGridPage(root.getAttribute("data-grid-page"));
  ...
  document.addEventListener("click", function (evt) {
    ...
    if (goBtn) {
      ...
      if (proot) {
        evt.preventDefault();
        var parsedCur = parseGridPage(proot.getAttribute("data-grid-page"));
        ...
        if (goBtn.hasAttribute("data-grid-page-prev"))
          ...
          var parsedTo = parseGridPage(goBtn.getAttribute("data-grid-goto"));
 ...
 *             blur restores from the native (date.js). Same class as
...
(function () {
  ...
  function buildEditor(edit) {
    ...
    if (edit.kind === "bool") {
      ...
      // (cycle 2150). Compose Field date so date.js owns parse/validity.
      el = document.createElement("span");
      el.className = "inline-edit-date";
      el.setAttribute("data-date-group", "");
      var native = document.createElement("input");
      native.type = "date";
      native.className = "inline-edit-input inline-edit-date-native";
      native.value = edit.value;
      native.setAttribute("aria-label", "Edit " + (edit.label || edit.colKey));
      var iso = document.createElement("input");
      iso.type = "text";
      iso.className = "inline-edit-input inline-edit-date-iso";
      iso.setAttribute("data-date-iso", "");
      ...
      // (cycle 2153). Compose Field time so time.js owns parse/validity.
      var clock = clockType(edit);
      var seed = normalizeClockValue(edit.value, clock);
      el = document.createElement("span");
      el.className = "inline-edit-time";
      el.setAttribute("data-time-group", "");
      var tNative = document.createElement("input");
      tNative.type = clock;
      tNative.className = "inline-edit-input inline-edit-time-native";
      tNative.value = seed;
      tNative.setAttribute("aria-label", "Edit " + (edit.label || edit.colKey));
      var tIso = document.createElement("input");
      tIso.type = "text";
      tIso.className = "inline-edit-input inline-edit-time-iso";
      tIso.setAttribute("data-time-iso", "");
  ...
  function editorHost(el) {
    return el && el.closest ? el.closest("[data-grid-editor]") : null;
  }
  ...
  function dateLeftoverBlocksCommit(root, target) {
    ...
    var iso = editor.querySelector("[data-date-iso]");
  ...
  function timeLeftoverBlocksCommit(root, target) {
    ...
    var native =
      editor.querySelector('input[type="datetime-local"]') ||
      editor.querySelector('input[type="time"]');
    var iso = editor.querySelector("[data-time-iso]");
    if (target && native && (target === native || native.contains(target))) {
      return false;
    }
    if (!iso) return false;
  ...
  function startEdit(root, span) {
    ...
    root._dzEdit = {
      ...
      clock: span.getAttribute("data-edit-clock") || "",
    };
    openEditor(root);
  }
...
(function () {
  ...
  function dropStackFrom(el, board) {
    ...
    var stack = el.closest("[data-kanban-stack][data-to-state]");
    if (stack && board.contains(stack)) return stack;
    var col = el.closest(".kanban-column");
    if (!col || !board.contains(col)) return null;
    stack = col.querySelector("[data-kanban-stack][data-to-state]");
 ...
 *
 * Timing (attrs on the root; namespaced form is `data-*`, gallery may strip):
 *   - data-blur-grace-ms (default 200): after focus leaves, wait before
 *     closing so a result-row click (blur-then-click) can fire htmx.
 *   - data-confirm-hold-ms (default 1500): after a select exchange paints
 *     `.select-result-confirm`, keep the panel open this long so the
 *     confirmation is readable; then close. 0 = no hold (close with blur).
 *     Alias: data-confirm-dwell-ms (same meaning).
 *
...
(function () {
  ...
  function isResultRow(el) {
    return !!(
      el &&
      el.closest &&
      (el.closest(".search-result-row") || el.closest(".search-result-row"))
    );
  }
...
/* ── controllers/date-range.js ── */
/* HYPERPART: date-range */
/*
 * date-range — keep From/To honest before the hx-get fires.
 *
 * Contract:
 *   - root: `[data-date-range]` (gallery also accepts `[data-date-range]`)
 *   - from: `input[type=date][name=date_from]`
 *   - to:   `input[type=date][name=date_to]`
 *   - iso:  `[data-date-iso]` companion in the same `.date-range-group`
...
(function () {
  ...
  function rootOf(el) {
    if (!el || !el.closest) return null;
    return (
      el.closest("[data-date-range]") || el.closest("[data-date-range]")
    );
  }

  function isoOf(dateEl) {
    if (!dateEl || !dateEl.parentElement) return null;
    return dateEl.parentElement.querySelector("[data-date-iso]");
  }
  ...
  function onBoundChange(evt) {
    ...
    if (t.getAttribute && t.hasAttribute("data-date-iso")) {
  ...
  document.addEventListener(
    ...
    function (evt) {
      var t = evt.target;
      if (!t || !t.hasAttribute || !t.hasAttribute("data-date-iso")) {
        return;
      }
      var root = rootOf(t);
      if (!root) return;
...
/* ── controllers/search-box.js ── */
/* HYPERPART: search-box */
/*
 * search-box — empty query must not hx-get a silent / fake result list.
 *
 * Contract:
 *   - root: `[data-search-box]` (gallery also accepts `[data-search-box]`)
 *   - input: `input[type=search]`
 *   - results: `.search-box-results` / `.search-box-results`
 *
...
(function () {
  ...
  function rootOf(el) {
    if (!el || !el.closest) return null;
    return (
      el.closest("[data-search-box]") || el.closest("[data-search-box]")
    );
  }
...
/* HYPERPART: field */
/*
 * color — swatch ↔ hex companion for a colour group.
 *
 * Contract:
 *   - root: `[data-color-group]` (also class `form-color-group`)
 *   - swatch: `.form-color-input` — native <input type=color>; this
 *             is the submitted value (the hex companion has no name)
 *   - hex: `.form-color-hex` — editable text (legacy: a span readout)
 *
...
(function () {
  ...
  function parts(el) {
    if (!el || !el.closest) return null;
    var group =
      el.closest("[data-color-group]") || el.closest(".form-color-group");
    if (!group) return null;
    return {
      group: group,
      color: group.querySelector(".form-color-input"),
      hex: group.querySelector(".form-color-hex"),
    };
  }
  ...
  document.addEventListener("input", function (evt) {
    var p = parts(evt.target);
    if (!p) return;
    if (evt.target.closest && evt.target.closest(".form-color-input")) {
      applyFromColor(p);
      return;
    }
    if (
      p.hex &&
      (evt.target === p.hex || evt.target.closest(".form-color-hex"))
    ) {
...
/* HYPERPART: field */
/*
 * time — native time / datetime-local ↔ ISO companion.
 *
 * Contract:
 *   - root: `[data-time-group]` (gallery also accepts `[data-time-group]`)
 *   - native: `input[type=time]` or `input[type=datetime-local]`
 *             — this is the submitted value (the companion has no name)
 *   - iso: `[data-time-iso]` — editable text (no name)
 *
...
(function () {
  ...
  function rootOf(el) {
    if (!el || !el.closest) return null;
    return (
      el.closest("[data-time-group]") || el.closest("[data-time-group]")
    );
  }
  ...
  function parts(el) {
    ...
    return {
      ...
      iso: group.querySelector("[data-time-iso]"),
    };
  }
...
/* HYPERPART: field */
/*
 * date — native date ↔ ISO companion.
 *
 * Contract:
 *   - root: `[data-date-group]` (gallery also accepts `[data-date-group]`)
 *   - native: `input[type=date]` — this is the submitted value
 *             (the companion has no name)
 *   - iso: `[data-date-iso]` — editable text (no name)
 *
...
(function () {
  ...
  function rootOf(el) {
    if (!el || !el.closest) return null;
    return (
      el.closest("[data-date-group]") || el.closest("[data-date-group]")
    );
  }
  ...
  function parts(el) {
    ...
    return {
      ...
      iso: group.querySelector("[data-date-iso]"),
    };
  }
...
/* ── controllers/number.js ── */
/* HYPERPART: field */
/*
 * number — native number ↔ editable companion.
 *
 * Contract:
 *   - root: `[data-number-group]` (gallery also accepts `[data-number-group]`)
 *   - native: `input[type=number]` — this is the submitted value
 *             (the companion has no name)
 *   - out: `[data-number-value]` — editable text (no name)
 *
...
(function () {
  ...
  function rootOf(el) {
    if (!el || !el.closest) return null;
    return (
      el.closest("[data-number-group]") || el.closest("[data-number-group]")
    );
  }
  ...
  function parts(el) {
    ...
    return {
      ...
      out: group.querySelector("[data-number-value]"),
    };
  }
...
/* HYPERPART: code */
/*
 * code — copy control for the code Hyperpart.
 *
 * Contract (mirrors contracts/code.py DOM_CONTRACT):
 *   [data-code] root
 *   [data-code-copy] optional button — copies .code__source textContent
...
(function () {
  ...
  function sourceFor(btn) {
    var root = btn.closest("[data-code]");
    if (!root) return null;
    ...
    return (
      root.querySelector(".code__source") ||
      root.querySelector(".code__pre code") ||
  ...
  document.addEventListener("click", function (e) {
    var btn = e.target && e.target.closest && e.target.closest("[data-code-copy]");
    if (!btn) return;
    e.preventDefault();
    e.stopPropagation();
...
/* ── controllers/details-light-dismiss.js ── */
/* HYPERPART: menu */
/* HYPERPART: popover */
/*
 * details-light-dismiss — configurable light-dismiss for overlay <details>.
 *
 * Stem: overlay-light-dismiss (spatial vs temporal abandon).
 *
 * Targets (class defaults when data-dismiss is omitted):
 *   details.menu / details.menu
 *   details.popover / details.popover
 *
 * Per-instance configuration (no CPU when closed — one timer max per open root):
 *
 *   data-dismiss="esc outside"     spatial: Escape + pointer outside (default)
 *   data-dismiss="outside"         spatial outside only (touch-first)
 *   data-dismiss="esc"             Escape only (discouraged alone)
 *   data-dismiss="none"            opt out — native toggle only
 *   data-dismiss-ms="4000"         temporal: auto-close after ms (while open)
 *                                     implies timeout; keeps default spatial unless
 *                                     data-dismiss="none"
 *
 *   Tokens may combine: data-dismiss="esc outside" data-dismiss-ms="5000"
 *
...
(function () {
  ...
  var ROOT =
    "details.menu, details.menu, details.popover, details.popover";
  ...
  /** Gallery unprefixes data-* → data-* on site/; apps keep data-*. */
  ...
  function parseMs(el) {
    var raw = attr(el, "data-dismiss-ms", "data-dismiss-ms");
    if (raw == null || raw === "") return 0;
    var n = parseInt(raw, 10);
  ...
  function policy(el) {
    var ms = parseMs(el);
    var raw = attr(el, "data-dismiss", "data-dismiss");
    if (raw != null) {
...
/* ── controllers/hover-card.js ── */
/* HYPERPART: hover-card */
/*
 * hover-card — open/close for coarse pointers (and explicit click).
 *
 ...
 *     a click/tap toggles data-open (gallery: data-open after prefix strip).
 *   - Outside pointer + Escape close the explicit open state.
 *
 * Contract:
 *   Root:    [data-hover-card] | [data-hover-card] | .hover-card | .hover-card
 *   Trigger: .hover-card__trigger | .hover-card__trigger
 *   Panel:   .hover-card__content | .hover-card__content
 *            (.hover-card__panel | .hover-card__panel legacy)
 *   Open:    data-open="" on root (unprefixed data-open on gallery)
...
(function () {
  ...
  var ROOT_SEL =
    "[data-hover-card], [data-hover-card], .hover-card, .hover-card";
  var TRIGGER_SEL = ".hover-card__trigger, .hover-card__trigger";
...
/* ── controllers/toggle.js ── */
/* HYPERPART: toggle */
/*
 * toggle — client press state for toolbar-style mode controls.
 *
 * Contract:
 *   Root: button (or role=button) with [data-toggle] | [data-toggle]
 *         class .toggle | .toggle is presentation only.
...
(function () {
  ...
  var ROOT_SEL =
    ...
    "[data-toggle][role='button'], [data-toggle][role='button']";
